...
=head1 ASSIGNMENT FUNCTIONS
    ...
    $obj = Math::FakeFloat16::new([$val]);
     ...
     $val can be an IV (integer), an NV (perl float), a PV (string),
     ...
     If necessary, $val will be rounded (to nearest, ties to even)
...
=head1 OUTPUT FUNCTIONS
   ...
   $obj = pack_f16_hex($string);
    Convert the string ($string) returned by unpack_f16_hex()
    into a Math::FakeFloat16 object whose value is the same as the
